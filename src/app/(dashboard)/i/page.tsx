...
export default function IPage() {
  ...
  const handleNotifyToggle = async () => {
    try {
      ...
      const res = await fetch("/api/feature-waitlist", {
        ...
        body: JSON.stringify({ feature_name: "influence", interested: next }),
      });
      if (!res.ok) throw new Error("Failed to update preference");
      setInterested(next);
      toast.success(next ? "You'll be notified when Influence goes live" : "Removed from notifications");
    } catch (e) {
  ...
  return (
    <div className="p-8">
      <PageHeader
        title="I — Influence"
        description="A hub for brand collaborations and marketplaces."
        breadcrumbs={[{ label: "Dashboard", href: "/x" }, { label: "Influence" }]}
      />
      <div className="mx-auto max-w-2xl">
        <Card className="rounded-2xl p-8 text-center">
          ...
          <p className="mt-3 text-secondary-700">
            Discover collaborators, manage offers, and grow partnerships — Influence is on the way.
          </p>
          <div className="mt-6">

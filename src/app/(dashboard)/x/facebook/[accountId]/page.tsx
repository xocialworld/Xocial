...
import { Facebook, ArrowLeft, RefreshCw, TrendingUp, Eye, ThumbsUp, MessageSquare, Share2, Upload } from 'lucide-react';
...
export default function FacebookPageDetailPage() {
    ...
    const handleSync = async () => {
        try {
            setSyncing(true);
            const response = await fetch(`/api/facebook/sync?accountId=${accountId}&type=full`, {
                method: 'POST',
            });
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="h-16 w-16 rounded-full overflow-hidden bg-[#1877F2] relative">
                ...
                <div className="flex gap-2">
                    <Button
                        variant="outline"
                        onClick={() => router.push(`/x/facebook/${accountId}/publish`)}
                    >
            ...
            <div className="grid gap-4 md:grid-cols-2">
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">
                            {formatNumber(account.metadata?.fan_count || account.follower_count || 0)}
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">Talking About</CardTitle>
                        <MessageSquare className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">
                            {formatNumber(account.metadata?.talking_about_count || 0)}
                        </div>
                    </CardContent>
                </Card>
            </div>
            ...
            <div className="flex gap-4">
                <Button
                    className="flex-1"
                    onClick={() => router.push(`/x/facebook/${accountId}/insights`)}
                >
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Your latest Facebook posts</CardDescription>
                </CardHeader>
                <CardContent>
                    {posts.length === 0 ? (
                        <p className="text-center text-muted-foreground py-8">
                            No posts found. Sync your page to import posts.
                        </p>
                    ) : (
                                ...
                                >
                                    <div className="flex-1">
                                        <p className="font-medium line-clamp-3">
                                            {post.content?.message || 'No message'}
                                        </p>
                                            ...
                                            {post.post_analytics?.[0] && (
                                                <>
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        <span>{formatNumber(post.post_analytics[0].reach || 0)}</span>
                                                    </div>
                                                    <div className="flex items-center gap-1">

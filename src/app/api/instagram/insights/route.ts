...
export async function GET(request: NextRequest) {
  try {
    ...
    const {
      ...
      .select('workspace_id, account_id, account_name, account_handle, access_token, follower_count, metadata, is_active')
      .eq('id', accountId)
    ...
    if (mediaId) {
      ...
      return NextResponse.json({
        ...
        scope: 'media',
        mediaId,
        metrics: formatInsightMetrics(insights),
      });
    }
    ...
    return NextResponse.json({
      ...
      account: {
        id: accountId,
        instagramId: account.account_id,
...
function formatInsightMetrics(insights: any): Record<string, number> {
  if (insights?.data && Array.isArray(insights.data)) {
    return insights.data.reduce((acc: Record<string, number>, metric: any) => {
      const value = metric?.values?.[0]?.value;
      acc[metric.name] = typeof value === 'number' ? value : Number(value) || 0;
      return acc;
    }, {});
  }

...
import { Facebook, AlertCircle, RefreshCw, ExternalLink, TrashIcon } from 'lucide-react';
...
export function FacebookAccountCard({
    ...
    const handleSync = async () => {
        setSyncing(true);
        try {
            const response = await fetch(`/api/facebook/sync?accountId=${account.id}&type=full`, {
                method: 'POST',
            });
            if (!response.ok) throw new Error('Sync failed');
            toast.success('Facebook page synced successfully');
            onSync?.(account.id);
        } catch (error) {
    ...
    const isExpiringSoon = tokenExpiresAt
        ? tokenExpiresAt.getTime() - now.getTime() < 7 * 24 * 60 * 60 * 1000
        : false;
    ...
    const getAccountStatus = () => {
        ...
        return { label: 'Active', variant: 'success' as const, icon: Facebook };
    };
            ...
            <CardHeader>
                <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                        <div className="h-12 w-12 rounded-full overflow-hidden bg-[#1877F2] flex items-center justify-center">
            ...
            <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-center">
                    <div>
                        <p className="text-2xl font-bold">
                            {formatNumber(account.metadata?.fan_count || account.follower_count)}
                        </p>
                ...
                {isExpiringSoon && !isExpired && (
                    <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <p className="text-sm text-yellow-800">
                            ...
                            Access token expires soon. Consider reconnecting your page.
                        </p>
                    </div>
                )}
                ...
                <div className="flex gap-2">
                    ...
                    <Button
                        ...
                        size="sm"
                        className="flex-1"
                        onClick={() => router.push(`/x/facebook/${account.id}`)}
                    >

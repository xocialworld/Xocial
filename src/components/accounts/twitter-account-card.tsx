...
import { Twitter, AlertCircle, RefreshCw, ExternalLink, TrashIcon } from 'lucide-react';
...
export function TwitterAccountCard({
    account,
    onSync,
    onDisconnect,
    className,
}: TwitterAccountCardProps) {
    ...
    const handleSync = async () => {
        setSyncing(true);
        try {
            const response = await fetch(`/api/twitter/sync?accountId=${account.id}&type=full`, {
                method: 'POST',
            });
    ...
    const getAccountStatus = () => {
        ...
        return { label: 'Active', variant: 'success' as const, icon: Twitter };
    };
            ...
            <CardHeader>
                <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                        <div className="relative h-12 w-12 rounded-full overflow-hidden bg-black flex items-center justify-center">
            ...
            <CardContent className="space-y-4">
                ...
                <div className="flex gap-2">
                    ...
                    <Button
                        ...
                        size="sm"
                        className="flex-1"
                        onClick={() => router.push(`/x/twitter/${account.id}`)}
                    >

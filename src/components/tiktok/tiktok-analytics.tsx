...
export function TikTokAnalytics() {
    ...
    const fetchAnalytics = useCallback(async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/tiktok/analytics?accountId=${accountId}&days=${timeRange}`);
        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Based on views</CardDescription>
                </CardHeader>
                <CardContent>
                            ...
                            >
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="font-medium truncate">
                                            {post.content?.title?.slice(0, 60) || post.content?.description?.slice(0, 60) || 'No title'}
                                        </p>
                                ...
                                <div className="flex items-center gap-6 text-sm">
                                    <div className="flex items-center gap-1">
                                        <Eye className="h-4 w-4 text-muted-foreground" />
                                        <span>{formatNumber(post.analytics?.views || 0)}</span>
                                    </div>
                                    <div className="flex items-center gap-1">
                                        <Heart className="h-4 w-4 text-muted-foreground" />

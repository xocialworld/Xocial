...
import { getInstagramAuthUrl } from '../instagram';
import { sanitizeOAuthRedirect } from '../redirect';

describe('Meta OAuth helpers', () => {
  it('builds Instagram via Facebook Login for Business with config ID and the Instagram callback', () => {
      ...
      getInstagramAuthUrl(
        {
          ...
          configurationId: 'ig-login-config',
        },
        'state-value'
      )
    );
    ...
    expect(authUrl.searchParams.get('config_id')).toBe('ig-login-config');
    expect(authUrl.searchParams.get('override_default_response_type')).toBe('true');
    expect(authUrl.searchParams.has('scope')).toBe(false);
  });

  it('builds classic Instagram via Facebook Login with Phase 1 scopes when no config ID is configured', () => {
    const authUrl = new URL(
      getInstagramAuthUrl(
        {
          clientId: 'facebook-app-id',

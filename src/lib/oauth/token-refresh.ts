...
export async function refreshMetaToken(accountId: string): Promise<TokenRefreshResult> {
  ...
  try {
    ...
    if (account.platform === 'instagram' && metadata.connected_via === 'instagram_login') {
      const currentInstagramToken = account.refresh_token
        ? decryptTokenIfNeeded(account.refresh_token)
        : decryptTokenIfNeeded(account.access_token);
      const refreshedInstagramToken = await refreshInstagramLongLivedToken(currentInstagramToken);
      const expiresAt = new Date(
        Date.now() + (refreshedInstagramToken.expires_in || 5184000) * 1000
      );
      const encryptedInstagramToken = encryptToken(refreshedInstagramToken.access_token);

      await supabase
        .from('social_accounts')
        .update({
          access_token: encryptedInstagramToken,
          refresh_token: encryptedInstagramToken,
    ...
    const pageId = account.platform === 'facebook' ? account.account_id : metadata.facebook_page_id;
